...
parseFunCall funName l = case parseAExp l of
  ParseSuccess (exp, (TComma _:rest)) ->
    case parseFunCall funName rest of
      ParseSuccess (FunCall funName args, trest) ->
        ParseSuccess (FunCall funName (exp:args), trest)
      ParseSuccess _ -> error "Internal parser error (parseFunCall)."
      ParseError pos -> ParseError pos
  ParseSuccess (exp, (TParenRight _:rest)) ->
    ParseSuccess (FunCall funName [exp], rest)
  ParseError pos -> ParseError pos
...
parseArgNameList (TVar argName pos : TParenRight _ : rest) =
  ParseSuccess ([argName], rest)
parseArgNameList (TVar argName pos : TComma _ : rest) =
  case parseArgNameList rest of
...
parseFunctionBody (TReturn _ : rest) =
...
parseFunctionBody l = case parseInstruction l of
  ...
  ParseSuccess (instr, rest) -> case parseFunctionBody rest of
    ParseSuccess (body, result, trest) ->
      ParseSuccess (instr : body, result, trest)
    ParseError pos -> ParseError pos
  ParseError pos -> ParseError pos

parseFunction :: [Token] -> MaybeParseErr ((String, Function), [Token])
parseFunction (TFunc pos : TVar funname _ : TParenLeft _ : trest) =
  case parseArgNameList trest of
    ParseError pos' -> ParseError pos'
    ParseSuccess (argNames, []) -> ParseError pos
    ParseSuccess (argNames, rest) ->
      case parseFunctionBody rest of
        ParseError pos -> ParseError pos
        ParseSuccess (body, result, rest) ->
          ParseSuccess ((funname, Function argNames body result), rest)
...
parseProgram l = case parseFunction l of
  ParseError pos -> ParseError pos
  ParseSuccess ((fun, funName), trest) ->
    case parseProgram trest of
      ParseError pos -> ParseError pos
