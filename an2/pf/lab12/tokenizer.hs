...
getInteger ((c:cs), pos) | isDigit c = let (cn, ps) = getInteger (cs, nextCol pos) in (c : cn, ps)-- TODO: fill in this case
getInteger ps = ("", ps)
...
tokenNext ps@(s@(c:rs), pos)
  ...
  | c == ':' = case rs of
      ...
      _ -> TokError pos
  | c == '=' = case rs of -- TODO: fill in this case
                 ('=':rstail) -> TokSuccess (TEq pos, (rstail, nextCol (nextCol pos)))
                 _ -> TokSuccess (TAssign pos, (rs, nextCol pos))
  | c == '<' = case rs of
