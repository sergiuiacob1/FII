...
liftIntegerOp f v1 v2 =
  case (v1, v2) of
    (Just (IntegerValue x1), Just (IntegerValue x2)) ->
        case f x1 x2 of
          Nothing -> Nothing
...
eqVal v1 v2 =
  case (v1, v2) of
    (Just (IntegerValue x1), Just (IntegerValue x2)) ->
      Just (BooleanValue (x1 == x2))
    (Just (BooleanValue b1), Just (BooleanValue b2)) ->
      Just (BooleanValue (b1 == b2))
    _ -> Nothing
...
liftBoolOp f v1 v2 =
  case (v1, v2) of
    (Just (BooleanValue x1), Just (BooleanValue x2)) ->
      Just (BooleanValue (f x1 x2))
    _ -> Nothing
...
evalExpList (hd:tl) valp funcval = case evalExp hd valp funcval of -- TODO: fill in this case
    Nothing -> Nothing
    Just val -> case evalExpList tl valp funcval of
                  Nothing -> Nothing
...
evalExp (FunCall id args) valp funcval = case funcval id of -- TODO: fill in this case
    Nothing -> Nothing
    Just functie -> case evalExpList args valp funcval of
        Nothing -> Nothing
        Just vals -> executeFunction functie vals empty funcval
...
update :: ValuationPar -> String -> Maybe Value -> String -> Maybe Value
...
executeFunction (Function argNames body result) args valp funcval =
  case (updateAll valp argNames args) of
    Nothing -> Nothing
...
executeInstruction valuation (Cond be p1 p2) funcval = case evalExp be valuation funcval of -- TODO: fill in this case
    Nothing -> Nothing
    Just val -> case val of 
      BooleanValue boolVal -> case boolVal of
        True -> executeBlock valuation p1 funcval
        False -> executeBlock valuation p2 funcval
      IntegerValue intVal -> Nothing
