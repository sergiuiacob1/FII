...
getBExpBinOp :: Token -> Maybe (BExp -> BExp -> BExp)
getBExpBinOp token = case token of
  TAnd -> Just And
  TOr -> Just Or
  _ -> Nothing

parseBExp (TAnd:rest) = case parseBExp rest of
  Just (bexp, tokenRest) -> case parseBExp tokenRest of
    Just (bexp', tokenRest') -> Just ((And bexp bexp'), tokenRest')
    _ -> Nothing
  _ -> Nothing
parseBExp (TOr:rest) = case parseBExp rest of
  Just (bexp, tokenRest) -> case parseBExp tokenRest of
    Just (bexp', tokenRest') -> Just ((Or bexp bexp'), tokenRest')
    _ -> Nothing
  _ -> Nothing
parseBExp (TTrue:rest) = Just (TrueExp,  rest)
parseBExp (TFalse:rest) = Just (FalseExp, rest)
parseBExp (TNot:rest) = case (parseBExp rest) of
                      Just (u, v) -> Just (Not u, v)
                      _ -> Nothing
